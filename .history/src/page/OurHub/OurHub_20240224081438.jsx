...
const PopUp = ({ artworkInfo, onClose }) => {
  return (
    <div className="popup">
      <div className="popup-content">
        <span className="close" onClick={onClose}>&times;</span>
        <h2>{artworkInfo.title}</h2>
        <p>Artist: {artworkInfo.artist}</p>
        <p>Description: {artworkInfo.description}</p>
  ...
  const renderImages = (artworks) => {
    switch (justify) {
      ...
      {artworks.map((artwork, index) => (
        <Artwork key={index} artworkInfo={artwork} />
      ))}
